...
def db_reconnect():
    """
    Force a PostgreSQL reconnection attempt.

    Useful after a Railway network interruption or a cold DB restart.
    Returns the new connection state.
    """
    import database as _db
    ok = try_reconnect_postgres()
    try:
        count = len(_db.list_portfolios())
        return {"backend": _db._BACKEND, "ok": ok, "portfolio_count": count}
    except Exception as e:
        return {"backend": _db._BACKEND, "ok": False, "error": str(e)}
