...
import { RefreshCw, TrendingUp, TrendingDown, CheckCircle, AlertTriangle, Zap } from "lucide-react";
import { api } from "../api";
...
export default function Rebalance() {
  ...
  const analyze = async () => {
    ...
    try {
      ...
      toast.error((e as any)?.response?.data?.detail || "فشل التحليل");
    } finally {
      setAnalyzing(false);
    }
  };
      ...
      <div className="card flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-0.5">
            يقارن التوزيع الحالي بالأهداف المحددة ويحسب الصفقات اللازمة
          </p>
        </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="card text-center">
              ...
              <p className="text-lg font-bold text-white">
                ${analysis.effective_total.toLocaleString("en", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
              </p>
            </div>
            <div className="card text-center">
            ...
            <div className="card text-center">
              ...
              <p className={`text-lg font-bold ${Math.abs(analysis.allocations_sum - 100) < 1 ? "text-green-400" : "text-red-400"}`}>
                {analysis.allocations_sum.toFixed(1)}%
              </p>
            </div>
            <div className="card text-center">
              <p className="text-xs text-slate-500 mb-1">صفقات مطلوبة</p>
              <p className={`text-lg font-bold ${analysis.trades.length > 0 ? "text-red-400" : "text-green-400"}`}>
              ...
              {analysis.drift_report.map(item => (
                <div
                  ...
                  className={`flex items-center justify-between p-3 rounded-xl ${
                    item.needs_action ? "bg-red-500/5 border border-red-500/20" : "bg-dark-700"
                  }`}
                >
                  ...
                  <div className="flex items-center gap-4 text-sm">
                    ...
                    <span className={`font-semibold w-16 text-right ${
                      item.needs_action
                        ? item.drift_pct > 0 ? "text-red-400" : "text-green-400"
                        : "text-slate-500"
                    }`}>
                    ...
                    {item.needs_action && (
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                        item.drift_pct > 0 ? "bg-red-500/20 text-red-400" : "bg-green-500/20 text-green-400"
                      }`}>
          ...
          {analysis.trades.length > 0 ? (
            <div className="card">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-sm text-slate-400">
                  إجمالي: ${analysis.trades.reduce((s, t) => s + t.usdt_amount, 0).toFixed(2)}
                </span>
              </div>
                  ...
                  <div key={trade.symbol} className="flex items-center justify-between p-3 bg-dark-700 rounded-xl">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className={`text-xs ${trade.action === "sell" ? "text-red-400" : "text-green-400"}`}>
                          {trade.action === "sell" ? "بيع" : "شراء"}
                        </p>
                      </div>
                    </div>
              ...
              <div className="flex items-center gap-3 p-4 bg-yellow-500/5 border border-yellow-500/20 rounded-xl mb-4">
                ...
                <p className="text-sm text-yellow-300">
                  سيتم تنفيذ الصفقات بأسعار السوق الحالية. تأكد من صحة الإعدادات قبل المتابعة.
                </p>
              </div>
              <button
                onClick={execute}
            ...
            <div className="card flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-slate-400 mt-0.5">
                  جميع الأصول ضمن حد الانحراف المسموح به ({analysis.threshold}%)
                </p>
              </div>
            </div>
          )}
            ...
            {done.results?.map((r: any, i: number) => (
              <div key={i} className="flex items-center justify-between p-3 bg-dark-700 rounded-xl text-sm">
                <div className="flex items-center gap-2">
                  <span className={r.status === "ok" ? "text-green-400" : r.status === "error" ? "text-red-400" : "text-yellow-400"}>
                    {r.status === "ok" ? "✅" : r.status === "error" ? "❌" : "⏭"}
                  </span>
                  <span className="font-medium text-white">{r.symbol}</span>
                  <span className={`text-xs ${r.action === "sell" ? "text-red-400" : "text-green-400"}`}>
                    {r.action === "sell" ? "بيع" : "شراء"}
                  </span>
                </div>
                <span className="text-slate-400">
                  {r.status === "ok" ? `$${r.usdt?.toFixed(2)}` : r.reason || ""}
                </span>
              </div>
            ))}
          </div>
