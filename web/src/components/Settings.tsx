...
export default function Settings({ onSaved }: { onSaved: () => void }) {
  ...
  const save = async () => {
    if (Math.abs(totalPct - 100) > 0.1) { setMsg('❌ مجموع النسب يجب أن يساوي 100%'); return; }
    const symbols = assets.map(a => a.symbol.trim().toUpperCase());
    if (symbols.some(s => !s)) { setMsg('❌ أدخل رمز كل عملة'); return; }
    if (new Set(symbols).size !== symbols.length) { setMsg('❌ لا يمكن تكرار العملات'); return; }
  ...
  if (!cfg) return <div className="text-center text-gray-500 py-20 animate-pulse">جاري التحميل...</div>;

  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-white">الإعدادات</h1>
        <p className="text-gray-400 text-sm mt-1">تعديل إعدادات المحفظة الذكية</p>
      </div>

      {/* Bot name (read-only) */}
      <div className="card">
        <div className="label">اسم البوت (لا يمكن تغييره)</div>
        <div className="input bg-gray-800/50 text-gray-500 cursor-not-allowed">{cfg.bot.name}</div>
        <p className="text-xs text-gray-600 mt-1">لتغيير الاسم، أنشئ بوتاً جديداً من صفحة "إنشاء بوت"</p>
      </div>
        ...
        <div className="space-y-2">
          {assets.map((a, i) => (
            <div key={i} className="flex items-center gap-2">
              <input className="input w-28 font-mono uppercase" value={a.symbol}
                onChange={e => updateSymbol(i, e.target.value)} placeholder="BTC" maxLength={10} />
              <div className="flex-1 relative">
                <input type="number" min={0} max={100} step={0.1} className="input pl-8"
                  value={a.allocation_pct} onChange={e => updatePct(i, parseFloat(e.target.value) || 0)} />
      ...
      <div className="card space-y-4">
        ...
        {[
          { label: 'البيع عند الإنهاء', desc: 'تحويل كل الأصول إلى USDT عند إيقاف البوت', val: sellTerm, set: setSellTerm, color: 'bg-brand' },
          { label: 'تمكين تحويل الأصول', desc: 'استخدام رصيد المحفظة الحالي أولاً قبل الشراء', val: assetTransfer, set: setAssetTransfer, color: 'bg-brand' },
          { label: '🧪 وضع تجريبي', desc: 'تشغيل البوت بأموال وهمية', val: paperTrading, set: setPaperTrading, color: 'bg-yellow-500' },
